...
void FSA9480_MhlSwitchSel(bool sw)//Rajucm
{
	...
	else
	{
		...
		while(s5p_hpd_get_state()) ///Rajucm: here while is critical
		{
	...
	if(dev2)
	{
		switch(dev2)
		{
			...
			case FSA9480_DEV_TY2_AV:
				...
				printk(KERN_ERR "\nFSA MHL FSA9480_DEV_TY2_AV %x attach %x  FSA9480_AudioDockConnectionStatus %d  mhl_cable_status = %d\n",
									FSA9480_DEV_TY2_AV,attach, FSA9480_AudioDockConnectionStatus, mhl_cable_status);
				...
				{   
					...
					{
						/* Make sure the sii9234 and max8893 are up before invoking 	sii9234 initialization	*/
						if((SII9234_i2c_status == 1) && (max8893_i2cprobe_status == 1))
						{
						...
						{
							schedule_delayed_work(&sii9234_initialize_work, 2 * HZ);
						}
