...
#if defined (CONFIG_CP_CHIPSET_STE)
static int phone_hw_watchdog=0; 
static void m5720_on(struct modemctl *mc)
{	
	if(!phone_hw_watchdog)
	{
		dev_dbg(mc->dev, "%s\n", __func__);
	...
	else
	{
		...
		{
			if(mc->irq_int_resout)
				enable_irq(mc->irq_int_resout);
...
static void m5720_off(struct modemctl *mc)
{
	...
	{
		printk("[%s] Phone hw watchdog [%d]\n", __func__, phone_hw_watchdog);
	}
