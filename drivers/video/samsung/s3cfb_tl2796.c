...
#if defined(CONFIG_S5PC110_DEMPSEY_BOARD) || defined(CONFIG_S5PC110_CELOX_BOARD)
#define MAX_GAMMA_VALUE_LD9040	24
#endif
#define CRITICAL_BATTERY_LEVEL 5
...
static unsigned int g_Brightness_Level;  //hj2.choi
#endif
...
static void s6e63m0_c110_spi_write_byte(unsigned short command)
{
	int     j;
	unsigned short data;
...
{
#if defined(CONFIG_S5PC110_DEMPSEY_BOARD) || defined(CONFIG_S5PC110_CELOX_BOARD)
	s6e63m0_c110_spi_write_byte(reg);
#else
...
void tl2796_ldi_init(void)
{
	...
	{
		mdelay(10);
		s6e63m0_panel_send_sequence(ld9040_SETTING);
		s6e63m0_panel_send_sequence(ld9040_STANDBY_OFF);
		mdelay(170);
		//s6e63m0_panel_send_sequence(ld9040_DISPLAY_ON);
	}
	else
	{
...
#endif
	{
		s6e63m0_panel_send_sequence(ld9040_DISPLAY_OFF);
		s6e63m0_panel_send_sequence(ld9040_STANDBY_ON);
		mdelay(180);
	}
	else
	{
...
#endif
		{
			gamma_val_x10 = 10*(MAX_GAMMA_VALUE_LD9040-1)*bl/(MAX_BL-MIN_BL) + (10 - 10*(MAX_GAMMA_VALUE_LD9040-1)*(MIN_BL)/(MAX_BL-MIN_BL)) ;
...
#if defined(CONFIG_S5PC110_DEMPSEY_BOARD) 
static int ld9040_set_elvss()
{
	const unsigned short * elvss_mode;
	switch (g_Brightness_Level) {
		case 0: //30cd/m2		
		case 1: //40cd/m2		
		case 2: //70cd/m2
		case 3: //90cd/m2	
		case 4: //100cd/m2	
			elvss_mode = SEQ_ELVSS_35;
			break;
		case 5: //1//110cd/m2		
		case 6: //120cd/m2		
		case 7: //130cd/m2
		...
		case 9: //150cd/m2
		case 10: //160cd/m2 	
			elvss_mode = SEQ_ELVSS_39;
			break;
		case 11: //170cd/m2
		case 12: //180cd/m2
		case 13: //190cd/m2 
		case 14: //200cd/m2
			elvss_mode = SEQ_ELVSS_42;
			break;
		case 15: //210cd/m2
		case 16: //220cd/m2
		case 17: //230cd/m2 
		...
		case 23: //290cd/m2 
		case 24: //300cd/m2
			elvss_mode = SEQ_ELVSS_49;	
			printk("ld9040_set_elvss level  = %d\n",g_Brightness_Level);	
			break;	
		default:
			break;	
	}

	s6e63m0_panel_send_sequence(elvss_mode);
	return 0;
}
...
static void update_brightness(int gamma)
{
	...
	{
		if (on_19gamma)
			s6e63m0_panel_send_sequence(ld9040_p19Gamma_set[gamma]);
		else
		//	s6e63m0_panel_send_sequence(ld9040_p22Gamma_set[gamma]);
			s6e63m0_panel_send_sequence(ld9040_4DOT52_p22Gamma_set[gamma]);

		s6e63m0_panel_send_sequence(ld9040_gamma_update);
	}
	else
	{
...
#if defined(CONFIG_S5PC110_DEMPSEY_BOARD) 
	gamma_value = g_Brightness_Level = (bl*10)/106; 

	ret=ld9040_set_elvss();
	if (ret) 
	{
		printk("lcd ELVSS setting failed.\n");				
	}
